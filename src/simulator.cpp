    ...
    , paused_(true)
{
    ...
    pause_resume_sub_ = private_nh_->subscribe<std_msgs::Bool>(pause_topic, 1, &Simulator::pauseResumeCb, this);
...
//            tf::Transform rpose = rdata.second->getRobotPose();
//            if( env_->isRobotInCollision(rpose, rdata.second->getRobotRadius()) )
//            {
//                // navigation simulation has failed, cannot be in collision
//                // so return
//                // if you want, you can trigger the other threads to stop working
//                return;
//            }
            ...
            for(std::vector<Sensor::Ptr>::iterator sit = sensors.begin(); sit != sensors.end(); sit++)
            {
                ...
                publishSensorFOV(sensor_fov_vis_pubs_.at(sensor->getSensorFQName()), sensor);
            }
        }
...
Simulator::setUp()
{
    ...
    for(std::vector<std::string>::iterator rnit = robot_names.begin(); rnit != robot_names.end(); rnit++)
    {
        ...
        for(std::vector<std::string>::iterator snit = sensor_names.begin(); snit != sensor_names.end(); snit++)
        {
            ...
            std::string sensor_fov_pub_topic = sensor->getSensorFQName() + std::string("/fov");
            ros::Publisher sensor_fov_pub = private_nh_->advertise<visualization_msgs::Marker>(sensor_fov_pub_topic, 1, true);
            sensor_fov_vis_pubs_.insert(std::make_pair(sensor->getSensorFQName(), sensor_fov_pub));
        ...
        ros::Subscriber plan_sub = nh_->subscribe<nav_msgs::Path>(robot_name + "/plan", 1, &Navigator::planCallback, navigator.get());
        nav_cmd_subs_.insert(std::make_pair(robot_name, plan_sub));
        // for each navigator, advertise a topic to publish its status
        ros::Publisher nav_status_pub = nh_->advertise<NavigationStatus>(robot_name + "/plan_status", 1);
...
void
Simulator::pauseResumeCb(std_msgs::Bool msg)
{
    paused_ = msg.data;
}

void Simulator::publishSensorFOV(ros::Publisher &pub, Sensor::Ptr sensor)
{
    visualization_msgs::Marker marker;
    marker.header.frame_id = sensor->getSensorName();
    marker.header.stamp = ros::Time();
    marker.ns = sensor->getSensorName() + std::string("-fov");
    marker.id = 0;
    ...
    sensor->getSensorSpec().getSensorFOVCorners(fov_points);
    size_t k = 0;
    for(size_t i = 0, j = 4; i < 4; i++, j++)
    {
        if( i == 3 )
        {
            marker.points[k++] = fov_points[i];
            marker.points[k++] = fov_points[0];
            marker.points[k++] = fov_points[j];
            marker.points[k++] = fov_points[4];

            marker.points[k++] = fov_points[i];
            marker.points[k++] = fov_points[i+4];
        }
        else
        {
            marker.points[k++] = fov_points[i];
            marker.points[k++] = fov_points[i+1];
            marker.points[k++] = fov_points[j];
            marker.points[k++] = fov_points[j+1];
